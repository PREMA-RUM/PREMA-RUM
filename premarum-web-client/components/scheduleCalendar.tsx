    ...
    return courseOfferings.flatMap((val, index) => {
        ...
        return timeSlots.map((ts, index) => {
            ...
            return {
                ...
                start: new Date(`03-${(days as any)[ts.day] as string}-2022T${realStartTime}`),
                end: new Date(`03-${(days as any)[ts.day] as string}-2022T${realEndTime}`)
            }
        })
    })
}
        ...
        <StyledOuterBox> 
            <StyledInnerPaper>
                <Calendar
                    ...
                    toolbar={false}
                    formats={formats}
                    defaultDate={new Date('03-20-2022T00:00')} // default date to make week static on this time
                    events={dates}
